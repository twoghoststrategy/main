...
(function (global, factory) {
   ...
   */
    ...
    // cache original method
        ...
        def(arrayMethods, method, function mutator() {
            let args = []; let len = arguments.length
            while (len--) {
   ...
   */
    function assertProp(
        ...
        if (type) {
            ...
            for (let i = 0; i < type.length && !valid; i++) {
                let assertedType = assertType(value, type[i], vm)
                expectedTypes.push(assertedType.expectedType || ``)
                valid = assertedType.valid
            }
        }

        let haveExpectedTypes = expectedTypes.some(function (t) {
            return t 
        })
    ...
    let simpleCheckRE = /^(String|Number|Boolean|Function|Symbol|BigInt)$/
    ...
    let functionTypeCheckRE = /^\s*function (\w+)/
   ...
   */
    function getType(fn) {
        let match = fn && fn.toString().match(functionTypeCheckRE)
    ...
    let EXPLICABLE_TYPES = [`string`, `number`, `boolean`]
      ...
      typeof Proxy !== `undefined` && isNative(Proxy)
        ...
        let hasHandler = {
            has: function has(target, key) {
                ...
                let isAllowed = allowedGlobals(key) ||
   ...
   */
    Watcher.prototype.run = function run() {
        ...
        this.deep
                ...
                this.value = value
                if (this.user) {
                    let info = `callback for watcher "` + (this.expression) + `"`
   ...
   */
    ...
    // the object here.
        ...
        Vue.prototype.$watch = function (
            ...
            if (options.immediate) {
                let info = `callback for immediate watcher "` + (watcher.expression) + `"`
                pushTarget()
    ...
    let KeepAlive = {
        ...
        methods: {
            cacheVNode: function cacheVNode() {
                ...
                let cache = ref.cache
                let keys = ref.keys
                let vnodeToCache = ref.vnodeToCache
                let keyToCache = ref.keyToCache
                if (vnodeToCache) {
                    let tag = vnodeToCache.tag
                    let componentInstance = vnodeToCache.componentInstance
                    let componentOptions = vnodeToCache.componentOptions
                    cache[keyToCache] = {
    ...
    let isValidContentEditableValue = makeMap(`events,caret,typing,plaintext-only`)
   ...
   */
    ...
    let dynamicArgAttribute = /^\s*((?:v-[\w-]+:|@|:|#)\[[^=]+?\][^\s"'<>\/=]*)(?:\s*(=)\s*(?:"([^"]*)"+|'([^']*)'+|([^\s"'=<>`]+)))?/
        ...
        // 2.6 v-slot syntax
        {
            if (el.tag === `template`) {
                ...
                `<template> syntax when there are other named slots.`,
                    ...
                    let ref$1 = getSlotName(slotBinding$1)
                    let name$1 = ref$1.name
                    let dynamic$1 = ref$1.dynamic
                    let slotContainer = slots[name$1] = createASTElement(`template`, [], el)
   ...
   */
    ...
    function generate(
        ...
        let code = ast ? (ast.tag === `script` ? `null` : genElement(ast, state)) : `_c("div")`
        return {
    ...
    function checkEvent(exp, text, warn, range) {
        let stripped = exp.replace(stripStringRE, ``)
        let keywordMatch = stripped.match(unaryOperatorsRE)
