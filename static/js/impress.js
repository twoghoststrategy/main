...
(function (document, window) {
    ...
    let pfx = (function () {
        ...
        let prefixes = `Webkit Moz O ms Khtml`.split(` `)
        let memory = {}

        return function (prop) {
            if (typeof memory[ prop ] === `undefined`) {
                let ucProp = prop.charAt(0).toUpperCase() + prop.substr(1)
                let props = (prop + ` ` + prefixes.join(ucProp + ` `) + ucProp).split(` `)
    ...
    let validateOrder = function (order, fallback) {
        let validChars = `xyz`
        let returnStr = ``
        if (typeof order === `string`) {
            for (let i in order.split(``)) {
    ...
    // as second parameter.
    let rotate = function (r, revert) {
        let order = r.order ? r.order : `xyz`
        let css = ``
        let axes = order.split(``)
        if (revert) {
            axes = axes.reverse()
        }
    ...
    let scale = function (s) {
        return ` scale(` + s + `) `
    }

    // `computeWindowScale` counts the scale factor between window size and size
    // defined for the presentation in the config.
    let computeWindowScale = function (config) {
        let hScale = window.innerHeight / config.height
        let wScale = window.innerWidth / config.width
    ...
    // by default).
    let impress = window.impress = function (rootId) {
        ...
        // lastEntered step.
        let onStepLeave = function (currentStep, nextStep) {
        ...
        let init = function () {
            ...
            let meta = lib.util.$(`meta[name='viewport']`) || document.createElement(`meta`)
        ...
        let goto = function (el, duration, reason, origEvent) {
            reason = reason || `goto`
            ...
            let step = stepsData[ `impress-` + el.id ]
            ...
            if (activeStep && activeStep !== el) {
                let event = { target: activeStep, detail: {} }
        ...
        let prev = function (origEvent) {
            let prev = steps.indexOf(activeStep) - 1
        ...
        let next = function (origEvent) {
            let next = steps.indexOf(activeStep) + 1
        ...
        // Helper function
        let interpolate = function (a, b, k) {
            return a + (b - a) * k
        }
        ...
        let swipe = function (pct) {
            ...
            let nextElement = event.detail.next

            let nextStep = stepsData[ `impress-` + nextElement.id ]

            // If the same step is re-selected, force computing window scaling,
            let nextScale = nextStep.scale * windowScale
            let k = Math.abs(pct)
    ...
    let libraryFactories = {}
    impress.addLibraryFactory = function (obj) {
        for (let libname in obj) {
    ...
    var initLibraries = function (rootId) { //jshint ignore:line
        let lib = {}
        for (let libname in libraryFactories) {
    ...
    var execPreInitPlugins = function (root) { //jshint ignore:line
        for (let i = 0; i < preInitPlugins.length; i++) {
            let thisLevel = preInitPlugins[ i ]
            if (thisLevel !== undefined) {
                for (let j = 0; j < thisLevel.length; j++) {
                    thisLevel[ j ](root)
                }
            }
        }
    ...
    var execPreStepLeavePlugins = function (event) { //jshint ignore:line
        for (let i = 0; i < preStepLeavePlugins.length; i++) {
            let thisLevel = preStepLeavePlugins[ i ]
            if (thisLevel !== undefined) {
                for (let j = 0; j < thisLevel.length; j++) {
...
(function (document, window) {
    ...
    let roots = []
    let rootsCount = 0
    let startingState = { roots: [] }

    let libraryFactory = function (rootId) {
        if (roots[ rootId ]) {
            return roots[ rootId ]
        }
        ...
        let addEventListener = function (target, type, listenerFunction) {
        ...
        let pushCallback = function (callback) {
        ...
        let teardown = function () {
            ...
            for (i = 0; i < eventListenerList.length; i++) {
                let target = eventListenerList[ i ].target
                let type = eventListenerList[ i ].type
                let listener = eventListenerList[ i ].listener
    ...
    var recordStartingState = function (rootId) {
        ...
        let steps = document.getElementById(rootId).querySelectorAll(`.step`)
    ...
    var resetStartingState = function (rootId) {
        ...
        let activeId = root.querySelector(`.active`).id
...
(function (document, window) {
    ...
    let libraryFactory = function (rootId) {
        ...
        // window location.
        let getElementFromHash = function () {
        ...
        let getUrlParamValue = function (parameter) {
            let chunk = window.location.search.split(parameter + `=`)[ 1 ]
            let value = chunk && chunk.split(`&`)[ 0 ]
        ...
        // given as `fallback`).
        let toNumber = function (numeric, fallback) {
            return isNaN(numeric) ? (fallback || 0) : Number(numeric)
        }

        // `triggerEvent` builds a custom DOM event with given `eventName` and `detail` data
        // and triggers it on element given as `el`.
        let triggerEvent = function (el, eventName, detail) {
...
(function (document) {
    ...
    document.addEventListener(`impress:init`, function (event) {
        ...
        let autoplay = util.getUrlParamValue(`impress-autoplay`) || data.autoplay

        if (autoplay) {
            autoplayDefault = util.toNumber(autoplay, 0)
        }

        let toolbar = document.querySelector(`#impress-toolbar`)
        if (toolbar) {
    ...
    let makeDomElement = function (html) {
    ...
    var setButtonText = function () {
        if (toolbarButton) {
            ...
            let buttonWidth = toolbarButton.offsetWidth
            let buttonHeight = toolbarButton.offsetHeight
    ...
    var addToolbarButton = function (toolbar) {
        let html = `<button id="impress-autoplay-playpause" ` + // jshint ignore:line
...
(function (document, window) {
    ...
    let preInit = function () {
        if (window.markdown) {
            ...
            for (let idx = 0; idx < markdownDivs.length; idx++) {
                let element = markdownDivs[ idx ]
                let dialect = element.dataset.markdownDialect

                let slides = element.textContent.split(/^-----$/m)
...
(function (document) {
    ...
    document.addEventListener(`impress:init`, function (event) {
        ...
        let selectors = [`input`, `textarea`, `select`, `[contenteditable=true]`]
...
(function (document) {
    ...
    // Wait for impress.js to be initialized
    document.addEventListener(`impress:init`, function (event) {
        let api = event.detail.api
        let root = event.target
        let gc = api.lib.gc
        let util = api.lib.util
...
(function (document, window) {
    ...
    let goto = function (event) {
        ...
        if (data.gotoKeyList !== undefined &&
            ...
            let keylist = data.gotoKeyList.split(` `)
            let nextlist = data.gotoNextList.split(` `)
            ...
            if (keylist.length !== nextlist.length) {
                ...
                let index = keylist.indexOf(event.origEvent.key)
                if (index >= 0) {
                    let next = nextlist[ index ]
                    if (isNumber(next)) {
...
(function (document, window) {
    ...
    let renderHelpDiv = function () {
        let helpDiv = document.getElementById(`impress-help`)
        if (helpDiv) {
            let html = []
            for (let row in rows) {
                for (let arrayItem in row) {
...
(function (document, window) {
    ...
    let cssFileOldDefault = `css/impressConsole.css`
    let cssFile = undefined // jshint ignore:line

    // Css for styling iframs on the console
    let cssFileIframeOldDefault = `css/iframe.css`
    let cssFileIframe = undefined // jshint ignore:line
    ...
    // The console object
    let impressConsole = window.impressConsole = function (rootId) {
        rootId = rootId || `impress`
        ...
        let nextStep = function () {
            let classes = ``
            let nextElement = document.querySelector(`.active`)
        ...
        let onStepLeave = function () {
            if (consoleWindow) {
                // Set notes to next steps notes.
                let newNotes = document.querySelector(`.active`).querySelector(`.notes`)
                if (newNotes) {
                ...
                // Set the views
                let baseURL = document.URL.substring(0, document.URL.search(`#/`))
                let slideSrc = baseURL + `#` + document.querySelector(`.active`).id
                let preSrc = baseURL + `#` + nextStep().id
                let slideView = consoleWindow.document.getElementById(`slideView`)

                // Setting when already set causes glitches in Firefox, so check first:
                if (slideView.src !== slideSrc) {
                    slideView.src = slideSrc
                }
                let preView = consoleWindow.document.getElementById(`preView`)
        ...
        let onStepEnter = function () {
            if (consoleWindow) {
                ...
                let notes = consoleWindow.document.getElementById(`notes`)
        ...
        var triggerEventInView = function (frame, eventName, detail) {
            ...
            // http:// URL on Chrome.
            let event = frame.contentDocument.createEvent(`CustomEvent`)
            event.initCustomEvent(eventName, true, true, detail)
            frame.contentDocument.dispatchEvent(event)
        }

        let spaceHandler = function () {
            let notes = consoleWindow.document.getElementById(`notes`)
        ...
        let clockTick = function () {
            ...
            // Clock
            let clockStr = zeroPad(hours) + `:` + zeroPad(minutes) + `:` + zeroPad(seconds) +
                           ` ` + ampm
        ...
        let registerKeyEvent = function (keyCodes, handler, window) {
            if (window === undefined) {
        ...
        let open = function () {
            ...
            if (consoleWindow && !consoleWindow.closed) {
                ...
                if (consoleWindow == null) {
                    ...
                    let clickStr = `var x = document.getElementById('impress-console-button');` +
                                     `x.parentNode.removeChild(x);` +
                                     `var r = document.getElementById('` + rootId + `');` +
                                     `impress('` + rootId +
                                     `').lib.util.triggerEvent(r, 'impress:console:open', {})`
                    let styleStr = `margin: 25vh 25vw;width:50vw;height:50vh;`
        ...
        var resize = function () {
            ...
            let views = consoleWindow.document.getElementById(`views`)

            // SlideView may have a border or some padding:
            // asuming same border width on both direktions
            let delta = slideView.offsetWidth - slideView.clientWidth

            // Set views
            let slideViewWidth = (views.clientWidth - delta)
            let slideViewHeight = Math.floor(slideViewWidth * ratio)

            let preViewTop = slideViewHeight + preViewGap

            let preViewWidth = Math.floor(slideViewWidth * preViewDefaultFactor)
            let preViewHeight = Math.floor(slideViewHeight * preViewDefaultFactor)
        ...
        let _init = function (cssConsole, cssIframe) {
        ...
        let init = function (cssConsole, cssIframe) {
...
(function (document, window) {
    "use strict"
    let root; let api; let gc; let attributeTracker

    attributeTracker = []

    // Function names
    let enhanceMediaNodes
    let enhanceMedia
    let removeMediaClasses
    let onStepenterDetectImpressConsole
    let onStepenter
    let onStepleave
    let onPlay
    let onPause
    let onEnded
    let getMediaAttribute
    let teardown
    ...
    getMediaAttribute = function (attributeName, nodes) {
        let attrName; let attrValue; let i; let node
    ...
    enhanceMediaNodes = function () {
        let i; let id; let media; let mediaElement; let type
    ...
    enhanceMedia = function () {
        let steps; let stepElement; let i
        enhanceMediaNodes()
    ...
    onStepenter = function (event) {
        let stepElement; let media; let mediaElement; let i; let onConsole; let autoplay
        if ((!event) || (!event.target)) {
            return
        }
    ...
    onStepleave = function (event) {
        let stepElement; let media; let i; let mediaElement; let autoplay; let autopause; let autostop
        if ((!event || !event.target)) {
            return
        }
...
(function (document) {
    ...
    document.addEventListener(`impress:stepenter`, function (event) {
	      let oldprev = document.getElementsByClassName(`prev`)[ 0 ]
	      let oldnext = document.getElementsByClassName(`next`)[ 0 ]

	      let prev = getPrevStep(event.target)
	      prev.classList.add(`prev`)
	      let next = getNextStep(event.target)
...
(function (document) {
    ...
    document.addEventListener(`impress:init`, function (event) {
        ...
        let isNavigationEvent = function (event) {
...
(function (document) {
    ...
    let selectOptionsHtml = function () {
        let options = ``
        for (let i = 0; i < steps.length; i++) {
    ...
    let addNavigationControls = function (event) {
        api = event.detail.api
        let gc = api.lib.gc
        root = event.target
        steps = root.querySelectorAll(`.step`)

        let prevHtml = `<button id="impress-navigation-ui-prev" title="Previous" ` +
                         `class="impress-navigation-ui">&lt;</button>`
        let selectHtml = `<select id="impress-navigation-ui-select" title="Go to" ` +
                         `class="impress-navigation-ui">` + `\n` +
                           selectOptionsHtml() +
                           `</select>`
        let nextHtml = `<button id="impress-navigation-ui-next" title="Next" ` +
                         `class="impress-navigation-ui">&gt;</button>`
...
(function (document) {
    ...
    document.addEventListener(`impress:init`, function (event) {
        root = event.target
        getSteps()
        let gc = event.detail.api.lib.gc
        gc.pushCallback(function () {
            stepids = []
            if (progressbar) {
    ...
    function updateProgressbar(slideId) {
        let slideNumber = stepids.indexOf(slideId)
        if (null !== progressbar) {
            let width = 100 / (stepids.length - 1) * (slideNumber)
...
(function (document, window) {
     ...
     */
    let toNumberAdvanced = function (numeric, fallback) {
        if (typeof numeric !== `string`) {
            return toNumber(numeric, fallback)
        }
        let ratio = numeric.match(/^([+-]*[\d\.]+)([wh])$/)
        if (ratio == null) {
            return toNumber(numeric, fallback)
        } else {
            let value = parseFloat(ratio[ 1 ])
            let multiplier = ratio[ 2 ] === `w` ? window.innerWidth : window.innerHeight
            return value * multiplier
        }
    }

    let computeRelativePositions = function (el, prev) {
        let data = el.dataset
        ...
        if (data.relTo) {
            let ref = document.getElementById(data.relTo)
            if (ref) {
    ...
    let rel = function (root) {
        ...
        for (let i = 0; i < steps.length; i++) {
            ...
            let step = computeRelativePositions(el, prev)
    ...
    document.addEventListener(`impress:init`, function (event) {
        ...
        event.detail.api.lib.gc.pushCallback(function () {
            let steps = startingState[ root.id ]
            let step
            while (step = steps.pop()) {
...
(function (document, window) {
    ...
    let substep = function (event) {
        if ((!event) || (!event.target)) {
            return
        }

        let step = event.target
        let el // Needed by jshint
        if (event.detail.reason === `next`) {
    ...
    var showSubstepIfAny = function (step) {
        let substeps = step.querySelectorAll(`.substep`)
        if (substeps.length > 0) {
            let sorted = sortSubsteps(substeps)
            let visible = step.querySelectorAll(`.substep-visible`)
            return showSubstep(sorted, visible)
        }
    }

    var sortSubsteps = function (substepNodeList) {
        let substeps = Array.from(substepNodeList)
        let sorted = substeps
            .filter(el => el.dataset.substepOrder)
            .sort((a, b) => {
                let orderA = a.dataset.substepOrder
                let orderB = b.dataset.substepOrder
                return parseInt(orderA) - parseInt(orderB)
            })
            .concat(substeps.filter(el => el.dataset.substepOrder === undefined))
        return sorted
    }
    ...
    var showSubstep = function (substeps, visible) {
        if (visible.length < substeps.length) {
            ...
            let el = substeps[ visible.length ]
    ...
    var hideSubstepIfAny = function (step) {
        let substeps = step.querySelectorAll(`.substep`)
        let visible = step.querySelectorAll(`.substep-visible`)
        let sorted = sortSubsteps(visible)
        if (substeps.length > 0) {
    ...
    var hideSubstep = function (visible) {
        if (visible.length > 0) {
            ...
            let el = visible[ visible.length - 1 ]
...
(function (document, window) {
    ...
    let lastX = 0
    let lastDX = 0
    let threshold = window.innerWidth / 20
...
(function (document) {
    "use strict"
    let toolbar = document.getElementById(`impress-toolbar`)
    let groups = []
     ...
     */
    let getGroupElement = function (index) {
        let id = `impress-toolbar-group-` + index
        if (!groups[ index ]) {
            groups[ index ] = document.createElement(`span`)
            groups[ index ].id = id
            let nextIndex = getNextGroupIndex(index)
    ...
    if (toolbar) {
        ...
        toolbar.addEventListener(`impress:toolbar:appendChild`, function (e) {
            let group = getGroupElement(e.detail.group)
